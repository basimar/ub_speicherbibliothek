          ...
          switch ($runtype)
            ...
            case indi:
              ...
              if ( $indi_import == '0' ) then
                 ...
                 sqlplus dsv51/dsv51 @korr-item_indi.sql $datum_timestamp
              ...
              breaksw
            case kass:
              mv speibi_$listtype[1].dat barcodes_kass.dat
              sqlldr dsv51/dsv51 control=load_barcodes_kass
              sqlldr dsv51/dsv51 control=load_difflist_kass
              set kass_import = `grep 'data errors' load_barcodes_kass.log | awk '{print $1}'`
              if ( $kass_import == '0' ) then
                 echo "2.: Mutationen $filehandle ausfuehren" >> $log
                 sqlplus dsv51/dsv51 @korr-item_kass.sql $datum_timestamp
                 mv $inputdir/$filehandle $inputdir/$filehandle.done
              else
                 echo "Import von $filehandle gescheitert, Fehlerliste:" >> $log
                 cat load_barcodes_kass.bad >> $log
                 mv $inputdir/$filehandle $inputdir/$filehandle.error
                 set filehandle = "Fehler in Input Typ $listtype[1]"
                 set success = 'Abbruch'
              endif
              breaksw
            case koll:
              mv speibi_$listtype[1].dat barcodes_koll.dat
              sqlldr dsv51/dsv51 control=load_barcodes_koll
              sqlldr dsv51/dsv51 control=load_difflist_koll
              set koll_import = `grep 'data errors' load_barcodes_koll.log | awk '{print $1}'`
              if ( $koll_import == '0' ) then
                 echo "2.: Mutationen $filehandle ausfuehren" >> $log
                 sqlplus dsv51/dsv51 @korr-item_koll.sql $datum_timestamp
                 # sql zu erstellen fuer koll
                 mv $inputdir/$filehandle $inputdir/$filehandle.done
              else
                 echo "Import von $filehandle gescheitert, Fehlerliste:" >> $log
                 cat load_barcodes_koll.bad >> $log
...
# ------------------------------------------
 ...
 if ( -e $inputdir/speibi_koll_$datum.csv.done ) then
  sqlplus dsv51/dsv51 @speibi_stamm_koll.sql
  echo $line >> $log
  echo "Stammdaten Kollektivbestand um $end_time erzeugt." >> $log
 else
  echo $line >> $log
  echo "     Keine Stammdaten Kollektivbestand um $end_time" >> $log
 endif
 if ( -e $inputdir/speibi_kass_$datum.csv.done ) then
  sqlplus dsv51/dsv51 @speibi_stamm_kass.sql
  echo $line >> $log
  echo "Stammdaten Kassationsbestand um $end_time erzeugt." >> $log
 else
  echo $line >> $log
  echo "     Keine Stammdaten Kassationsbestand um $end_time" >> $log
 endif
